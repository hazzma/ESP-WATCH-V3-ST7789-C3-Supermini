...
static void build_menu_sprite(TFT_eSprite& spr, AppState st, int bx, int by) {
    ...
    // [UI AGENT] PREVIEW RENDER: Draw miniature dashboard during transition
    if (st == STATE_EXEC_STEPS) {
        spr.drawCircle(MENU_BW/2, MENU_BH/2 + 20, 35, 0x18C3); // Ring Base Preview
        spr.drawString(title, MENU_BW/2, 15);
    } else if (strlen(title) > 0) {
        spr.drawString(title, MENU_BW/2, is_set ? 20 : MENU_BH/2 - 5);
    }
    
    if (strlen(sub) > 0 && st != STATE_EXEC_STEPS) { spr.setTextSize(1); spr.setTextColor(TFT_GOLD); spr.drawString(sub, MENU_BW/2, MENU_BH - 20); }
...
static void animate_slide_transition(AppState from, AppState to, int dir) {
    TFT_eSPI& tft = display_hal_get_tft();
    power_manager_set_freq(FREQ_HIGH); // Turbo for transition
    ...
    // [UI AGENT] RAM OPTIMIZATION: 
    // Instead of creating new sprites, we reuse menu_spr (200x100).
    // We render the transition in slices to avoid huge dynamic allocation.
    const uint16_t* wall = assets_get_wallpaper(); 
    ...
    for (int i = 0; i <= steps; i++) {
        ...
        // Basic full-screen slice push (Stable)
        TFT_eSprite combo = TFT_eSprite(&tft);
        if (!combo.createSprite(240, MENU_BH)) return;
        combo.setSwapBytes(true);
        combo.pushImage(0, -90, 240, 280, wall);
        
        if (dir > 0) { 
            // SYNC BACKGROUND: X coordinate must match screen position for static wallpaper effect
