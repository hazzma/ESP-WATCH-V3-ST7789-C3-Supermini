...
#define REG_INT_MOTION_0    0x5F  // threshold[7:2] + duration[1:0]
// 0x60 jangan disentuh (no-motion threshold, biarkan default)
#define REG_CMD          0x7E
...
bool bmi160_hal_init() {
    ...
    if (!is_wake) {
        ...
        uint8_t sens = power_manager_get_rtw_sensitivity();
        if (sens == 0) {        // High
            writeReg(0x5F, (0x0A << 2) | 0x01); // 39mg, 2 samples
        } else if (sens == 1) { // Medium
            writeReg(0x5F, (0x14 << 2) | 0x01); // 78mg, 2 samples
        } else {                // Low
            writeReg(0x5F, (0x28 << 2) | 0x03); // 156mg, 4 samples
        }
        delay(1);
        ...
        if (rtw_mode == 1) {
            ...
            writeReg(REG_INT_EN_0, 0x07); // Enable Any-motion (X/Y/Z)
        }
        delay(1);
