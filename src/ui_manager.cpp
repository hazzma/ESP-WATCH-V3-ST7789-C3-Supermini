...
static void push_top_clock(bool force) {
    ...
    if (current_state == (int)STATE_TIMER_ALARM || current_state == STATE_EXEC_TIMER || (current_state >= STATE_SET_TIMER_H && current_state <= STATE_SET_TIMER_S) || current_state == STATE_EXEC_STOPWATCH || current_state == STATE_EXEC_HR) return; 
...
static void draw_menu_card(TFT_eSprite& spr, AppState st, int x, int y) {
    ...
    switch(st) {
        ...
        case STATE_EXEC_SETTINGS:   title = "SETTINGS";   sub = "[LIST]"; break;
        case STATE_SET_AOD_CONFIG:  title = "AOD CONFIG"; sub = "[LIST]"; break;
        case STATE_SET_RAISE2WAKE:  title = "RAISE2WAKE"; sub = "[LIST]"; break;
...
static void animate_slide_transition(AppState from, AppState to, int dir) {
    ...
    // [UI AGENT] Universal Border Scrub: Clean ghost zones if moving from/to full-height states
    bool is_full_from = (from == STATE_EXEC_STEPS || from == STATE_EXEC_SETTINGS || from == STATE_SET_AOD_CONFIG || from == STATE_SET_RAISE2WAKE);
    bool is_full_to   = (to == STATE_EXEC_STEPS || to == STATE_EXEC_SETTINGS || to == STATE_SET_AOD_CONFIG || to == STATE_SET_RAISE2WAKE);
    
    if (is_full_from || is_full_to) {
        const uint16_t* wall = assets_get_wallpaper();
        tft.pushImage(0, 25, 240, 65, (uint16_t*)(wall + (25 * 240)), 240); // Clean Title Zone (Clock Safe)
    ...
    int target_h = (to == STATE_EXEC_STEPS) ? 145 : 
                   (to == STATE_EXEC_SETTINGS || to == STATE_SET_AOD_CONFIG || to == STATE_SET_RAISE2WAKE) ? 160 :
                   ((to == STATE_SYNCING) ? 120 : MENU_BH);

    int target_y = (to == STATE_EXEC_STEPS || to == STATE_EXEC_SETTINGS || to == STATE_SET_AOD_CONFIG || to == STATE_SET_RAISE2WAKE) ? 60 : MENU_BY;
    int card_y_adj = (target_y == 90) ? 0 : (90 - target_y); // Compensate card position inside sprite

    for (int i = 0; i <= steps; i++) {
        int offset = (i * 240) / steps; 
        canvas_spr.pushImage(0, -target_y, 240, 280, wall); 
        
        if (dir > 0) { 
            draw_menu_card(canvas_spr, from, MENU_BX - offset, card_y_adj);
            draw_menu_card(canvas_spr, to, MENU_BX + 240 - offset, card_y_adj);
        } else { 
            draw_menu_card(canvas_spr, from, MENU_BX + offset, card_y_adj);
            draw_menu_card(canvas_spr, to, MENU_BX - 240 + offset, card_y_adj);
        }
        push_sub_sprite(0, target_y, 240, target_h);
...
void ui_manager_update() {
    ...
    if (bt == BTN_RIGHT_CLICK) {
        if (current_state == STATE_SET_BRIGHTNESS) { brightness_ui_val += 15; if (brightness_ui_val > 255) brightness_ui_val = 255; display_hal_backlight_set(brightness_ui_val); nd = true; }
            ...
            else if (current_state == STATE_MENU_STOPWATCH) target = STATE_MENU_SYNC;
            else if (current_state == STATE_MENU_SYNC)      target = STATE_MENU_SETTINGS;
        ...
        if (current_state == STATE_SET_TIMER_H) { timer_h--; if (timer_h < 0) timer_h = 23; nd = true; }
        ...
        else if (current_state == STATE_SET_AOD_BRIGHTNESS) { brightness_aod_val -= 15; if (brightness_aod_val < 15) brightness_aod_val = 15; nd = true; }
            ...
            else if (current_state == STATE_MENU_SETTINGS)   target = STATE_MENU_SYNC;
            else if (current_state == STATE_MENU_SYNC)       target = STATE_MENU_STOPWATCH;
        ...
        if      (current_state == STATE_EXEC_STEPS) { bmi160_hal_reset_steps(); nd = true; } // Reset ONLY in dash
            ...
            else if (settings_menu_idx == 2) { target = STATE_MENU_SYNC; }
        ...
        else if (current_state == STATE_SET_AOD_CONFIG || current_state == STATE_SET_RAISE2WAKE || current_state == STATE_SET_AOD_BRIGHTNESS) target = STATE_EXEC_SETTINGS;
...
static void render_current_state() {
    ...
    if (sjc && !is_full_black_mode) {
        if (from_full_black || last_rendered_state == STATE_WATCHFACE || (int)last_rendered_state == -1) {
    ...
    switch ((int)current_state) {
        ...
        case STATE_SET_TIMER_H: case STATE_SET_TIMER_M: case STATE_SET_TIMER_S:
            ...
            tft.setTextDatum(MC_DATUM); tft.setTextColor(TFT_WHITE, TFT_BLACK); tft.setTextSize(2); tft.drawString("TIMER SETUP", 120, 42);
        ...
        case STATE_SET_TIMEOUT:
            if (last_rendered_state != STATE_SET_TIMEOUT) {
                tft.pushImage(0, 25, 240, 65, (uint16_t*)(assets_get_wallpaper() + 25*240)); // Title Clean
                tft.pushImage(0, 190, 240, 90, (uint16_t*)(assets_get_wallpaper() + 190*240)); // Scrub List
        ...
        case STATE_SET_BRIGHTNESS:
            if (last_rendered_state != STATE_SET_BRIGHTNESS) {
                tft.pushImage(0, 25, 240, 65, (uint16_t*)(assets_get_wallpaper() + 25*240)); // Title Clean
                canvas_spr.pushImage(-MENU_BX, -MENU_BY, 240, 280, assets_get_wallpaper());
        ...
        case STATE_EXEC_SETTINGS: {
            if (last_rendered_state != STATE_EXEC_SETTINGS) {
                tft.pushImage(0, 25, 240, 65, (uint16_t*)(assets_get_wallpaper() + 25*240)); // Title Clean
                tft.pushImage(0, 220, 240, 60, (uint16_t*)(assets_get_wallpaper() + 220*240)); // Bottom Clear
                tft.setTextDatum(MC_DATUM); tft.setTextColor(TFT_WHITE); tft.setTextSize(2);
                tft.drawString("SETTINGS", 120, 42);
        ...
        case STATE_SET_AOD_CONFIG: {
            if (last_rendered_state != STATE_SET_AOD_CONFIG) {
                ...
                tft.pushImage(0, 220, 240, 60, (uint16_t*)(assets_get_wallpaper() + 220*240)); // Bottom Clear
                tft.setTextDatum(MC_DATUM); tft.setTextColor(TFT_WHITE); tft.setTextSize(2);
                tft.drawString("AOD CONFIG", 120, 42);
                tft.setTextSize(1); tft.setTextColor(TFT_DARKGREY);
        ...
        case STATE_SET_AOD_BRIGHTNESS: {
            if (last_rendered_state != STATE_SET_AOD_BRIGHTNESS) {
                ...
                tft.pushImage(0, 220, 240, 60, (uint16_t*)(assets_get_wallpaper() + 220*240)); // Bottom Clear
                tft.setTextDatum(MC_DATUM); tft.setTextColor(TFT_WHITE); tft.setTextSize(2);
                tft.drawString("AOD BRIGHTNESS", 120, 42);
                tft.setTextSize(1); tft.setTextColor(TFT_DARKGREY);
        ...
        case STATE_SET_RAISE2WAKE: {
            if (last_rendered_state != STATE_SET_RAISE2WAKE) {
                ...
                tft.pushImage(0, 220, 240, 60, (uint16_t*)(assets_get_wallpaper() + 220*240)); // Bottom Clear
                tft.setTextDatum(MC_DATUM); tft.setTextColor(TFT_WHITE); tft.setTextSize(2);
                tft.drawString("RAISE TO WAKE", 120, 42);
                tft.setTextSize(1); tft.setTextColor(TFT_DARKGREY);
                tft.drawString("L-HOLD: BACK | R-HOLD: TOGGLE", 120, 250);
            }
            canvas_spr.pushImage(0, -60, 240, 280, assets_get_wallpaper());
        ...
        case STATE_EXEC_STEPS:
            {
                ...
                if (last_rendered_state != STATE_EXEC_STEPS) {
                    tft.pushImage(0, 25, 240, 65, (uint16_t*)(assets_get_wallpaper() + 25*240)); 
                    tft.setTextDatum(MC_DATUM); tft.setTextColor(TFT_WHITE); tft.setTextSize(2); 
                    tft.drawString("ACTIVITY", 120, 42);
                ...
                if (steps != last_disp_steps || last_rendered_state != STATE_EXEC_STEPS) {
                    ...
                    canvas_spr.pushImage(0, -205, 240, 280, assets_get_wallpaper()); // Surgical Crop at Y=205 (Corrected h=280)
        ...
        default:
            if (sjc) {
                // [UI AGENT] Silent Jump Cleanup: Wipe borders if jumping from list to card (no animation)
                bool from_full = (last_rendered_state == STATE_EXEC_STEPS || last_rendered_state == STATE_EXEC_SETTINGS || last_rendered_state == STATE_SET_AOD_CONFIG || last_rendered_state == STATE_SET_RAISE2WAKE);
                if (from_full) {
                    const uint16_t* wall = assets_get_wallpaper();
                    tft.pushImage(0, 25, 240, 65, (uint16_t*)(wall + (25 * 240)), 240); 
                    tft.pushImage(0, 190, 240, 90, (uint16_t*)(wall + (190 * 240)), 240); 
                }
            }
            canvas_spr.pushImage(-MENU_BX, -MENU_BY, 240, 280, assets_get_wallpaper());
    ...
    // [UI AGENT] Atomic Clock Refresh: Ensure time is updated without flicker
